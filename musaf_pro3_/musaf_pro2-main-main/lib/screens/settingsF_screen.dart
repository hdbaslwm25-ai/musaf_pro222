  ...
  Future<void> _loadPreferences() async {
    ...
    setState(() {
      ...
      soundAndVibrationEnabled =
          prefs.getBool('soundAndVibrationEnabled') ?? true;
    });
  }
        ...
        if (userDoc.exists && mounted) {
          setState(() {
            ...
            caregiverEmail =
                ...
                "لا يوجد بريد إلكتروني مسجل";
            caregiverPhone =
                userDoc.data()?['phoneNumber']?.toString().trim() ??
  ...
  Widget build(BuildContext context) {
    ...
    ImageProvider? profileImageProvider = caregiverImageUrl.isNotEmpty
        ? (caregiverImageUrl.startsWith('http')
              ? NetworkImage(caregiverImageUrl)
              : FileImage(File(caregiverImageUrl)))
        : null;
  ...
  void _handleDeleteRequest() {
    ...
    showDialog(
      ...
      builder: (ctx) => DeleteAccountDialog(
        authRepository:
            _authRepository, // نستخدم الـ Repository الموجود مسبقاً في الشاشة
      ),
    );
  }
        ...
        content: Column(
          ...
          children: [
            Icon(
              ...
              size: 60,
              color: hasPatient ? themeColor : Colors.grey,
            ),
            const SizedBox(height: 15),
            Text(
              hasPatient
                  ? "لديك مريض مرتبط حالياً."
                  : "لا يوجد مريض مرتبط بحسابك.",
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: hasPatient ? Colors.red : themeColor,
            ...
            child: Text(
              hasPatient ? "فك الارتباط" : "إضافة مريض",
              style: const TextStyle(
                fontFamily: 'Cairo',
                color: Colors.white,
                fontWeight: FontWeight.bold,
            ...
            onPressed: () async {
              if (nameController.text.isNotEmpty && currentUserId != null) {
                ...
                setState(() {
                  ...
                  caregiverPhone = phoneController.text.trim().isEmpty
                      ? "لا يوجد رقم هاتف مسجل"
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              ...
              if (caregiverEmail.isNotEmpty &&
                  caregiverEmail != "لا يوجد بريد إلكتروني مسجل" &&
                  !caregiverEmail.contains("جاري")) {
                try {
                  ...
                  if (mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(
                        content: Text(
                          "تم إرسال الرابط بنجاح! يرجى تفقد بريدك الإلكتروني 📧",
                          style: TextStyle(
                            fontFamily: 'Cairo',
                            fontWeight: FontWeight.bold,
                          ),
  ...
  void _showAboutAppDialog() {
    showAboutDialog(
      ...
      children: [
        ...
        const Text(
          'تطبيق مُسعف لمتابعة المرضى والرعاية الصحية.',
          style: TextStyle(fontFamily: 'Cairo'),
        ),
      ],
    );
